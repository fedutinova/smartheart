...
from rag_pipeline.ingestion import discover_files, load_and_clean, chunk_documents, build_chroma_index
...
from rag_pipeline.generation import build_prompt, build_llm, enhanced_query_with_llm
...
def main():
    ...
    if not files:
        print("Положите документы (PDF/Docx/Doc/TXT/MD/HTML/HTM/RTF) в папку ./documents и запустите снова.")
        return
    ...
    print("\n==== TOP RESULTS ====")
    for rank, (cid, score, doc, meta) in enumerate(zip(res.ids, res.combined_scores, res.documents, res.metadatas), start=1):
        v = res.vector_scores.get(cid, None)
        b = res.bm25_scores.get(cid, None)
        print(f"\n#{rank} id={cid}")
        print(f"combined={score:.6f} | vector_sim={(v if v is not None else float('nan')):.4f} | bm25={(b if b is not None else float('nan')):.4f}")
        print(f"source={meta.get('doc_name')} | chunk_index={meta.get('chunk_index')} | chunk_len={meta.get('chunk_len')}")
    ...
    if not api_key:
        print("Установите переменную окружения LLM_API_KEY для генерации ответа.")
        return
    prompt = build_prompt()
