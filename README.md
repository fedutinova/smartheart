...
Умное сердце — сервис для анализа ЭКГ по фотографиям бумажных плёнок. Пользователь загружает снимок, изображение обезличивается прямо в браузере (Tesseract.js + распознавание именованных сущностей), затем параллельно запускаются:
...
| `CV_SERVICE_URL` | — | Адрес `cv_service` (если пусто — классификация ритма отключена) |
